defmodule SymphonyElixir.Orchestrator do
  ...
  alias SymphonyElixir.{AgentRunner, CodingAgent, Config, Persistence, StatusDashboard, Telemetry, Tracker, Workspace}
  ...
  def init(_opts) do
    ...
    restored_totals = Persistence.load_agent_totals() || @empty_agent_totals
  ...
  defp schedule_issue_retry(%State{} = state, issue_id, attempt, metadata)
    ...
    Telemetry.emit_retry(%{issue_id: issue_id, attempt: next_attempt})
  ...
  defp apply_agent_token_delta(
    ...
    new_totals = apply_token_delta(agent_totals, token_delta)
    persist_async(fn -> Persistence.save_agent_totals(new_totals) end)
    %{state | agent_totals: new_totals}
  end
  ...
  def terminate(reason, %State{} = state) do
    ...
    if state.agent_totals do
      Persistence.save_agent_totals(state.agent_totals)
    end
